...
class FromLocalViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, SKPhotoBrowserDelegate {
    ...
    override func viewDidLoad() {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: 500.0, right: 0)
        setupTestData()
        setupCollectionView()
...
extension FromLocalViewController {
    ...
    @objc(collectionView:cellForItemAtIndexPath:) func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.exampleImageView.image = UIImage(named: "image\((indexPath as NSIndexPath).row % 16).jpg")
...
extension FromLocalViewController {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: IndexPath) -> CGSize {
        if indexPath.row == 0
        {
            return CGSize(width: UIScreen.main.bounds.size.width / 2 - 5, height: 120)
        }
...
private extension FromLocalViewController {
    ...
    func createLocalPhotos() -> [SKPhotoProtocol] {
        return (0..<16).map { (i: Int) -> SKPhotoProtocol in
            let photo = SKPhoto.photoWithImage(UIImage(named: "image\(i%16).jpg")!)
            //photo.caption = caption[i%30]
